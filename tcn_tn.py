...
class TCN(Layer):
    ...
    def compute_output_shape(self, input_shape):
        ...
        if not self.return_sequences:
            ...
            return [v if hasattr(v, 'value') else v for v in self.build_output_shape]
...
def compiled_tcn(num_feat,  # type: int
                 ...
                 name='tcn',  # type: str,
                 kernel_initializer=tf.initializers.random_uniform(-1, 1),  # type: str,
    ...
    def get_opt():
        if opt == 'adam':
            return Adam(lr=lr, clipnorm=1.)
        elif opt == 'rmsprop':
            return RMSprop(lr=lr, clipnorm=1.)
        else:
